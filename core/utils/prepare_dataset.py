...
def load_data(data_dir, dataset_str, seed=1234):
    ...
    if dataset_str == 'photo' or dataset_str == 'computers' or dataset_str == 'chameleon' or dataset_str == 'squirrel' or dataset_str == 'actor':
        ...
        if hasattr(nx, "from_numpy_matrix"):
            ...
            adj = nx.adjacency_matrix(nx.from_numpy_array(adj)).astype(np.float32)

        try:
            features = torch.Tensor(target_data.x.double())
        except TypeError:
            features = torch.Tensor(target_data.x.float())

        mask_list = [i for i in range(target_data.num_nodes)]
        train_mask_list, valid_mask_list, test_mask_list, target_data.train_node = get_split_new(seed, mask_list, target_data.y.numpy(), target_data.num_classes)
        ...
        idx_train, idx_val, idx_test, train_node = get_split_new(seed, all_idx, labels.numpy(), labels.numpy().max().item() + 1)
...
def get_split_new(seed, all_idx, labels, n_class):
    ...
    return idx_train, idx_val, idx_test, labeled_node
...
def prepare_datasets(config, fold=None, seed=None):
    if seed is not None:
        data_seed = seed
    elif fold is not None:
        data_seeds = [0, 42, 9897, 2442, 3645, 3147, 21, 89, 64, 57]
        data_seed = data_seeds[fold]
    else:
        data_seed = 42
    data = {}
    num_nodes = 0
    ...
    data = {'adj': adj.to(device) if device else adj,
            ...
            'train_mask': index2mask(idx_train, features.shape[0]).to(device) if device else index2mask(idx_train, features.shape[0]),
            'val_mask': index2mask(idx_val, features.shape[0]).to(device) if device else index2mask(idx_val, features.shape[0]),
            'test_mask': index2mask(idx_test, features.shape[0]).to(device) if device else index2mask(idx_test, features.shape[0])}
