...
def heat_diffusion_ind(graph, taus=TAUS, order = ORDER, proc = PROC):
    ...
    '''
    # Compute Laplacian
    # a = nx.adjacency_matrix(graph.cpu().detach().numpy())
    # a = nx.from_numpy_matrix(graph.cpu().detach().numpy())
    a = graph.cpu().detach().numpy()
    sparse_a, _ = pyg.utils.dense_to_sparse(graph.detach())
    n_nodes, _ = a.shape
    thres = np.vectorize(lambda x : x if x > 1e-4 * 1.0 / n_nodes else 0)
    lap = pyg.utils.get_laplacian(sparse_a)
    lap = pyg.utils.to_dense_adj(lap[0], edge_attr=lap[1]).squeeze()
    n_filters = len(taus)
    if proc == 'exact':
        ...
        for i in range(n_filters):
            ...
            heat[i] = sc.sparse.csc_matrix(heat[i])
    return heat, taus
