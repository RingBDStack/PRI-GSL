...
class GraphClf(nn.Module):
    def __init__(self, config):
        ...
        self.graph_module = 'gcn' # all of our models use gcn as our backbone
        self.device = config['device']
        nfeat = config['num_feat']
        nclass = config['num_class']
        hidden_size = config['hidden_size']
        self.dropout = config['dropout']
        self.graph_skip_conn = config.get('graph_skip_conn', True)
        self.graph_include_self = config.get('graph_include_self', True)
        self.scalable_run = config.get('scalable_run', False)

        if config.get('use_wave', False):
            # nfeat += config.get("wave_init_shape", 1000)
            input_hidden_size = hidden_size + config.get("wave_shape", 16)
        else:
        ...
        if self.graph_learn:
            ...
            self.graph_learner = graph_learn_fun(nfeat, config.get('graph_learn_hidden_size', 16),
                                            topk=config.get('graph_learn_topk', None),
                                            epsilon=config['graph_learn_epsilon'],
                                            num_pers=config.get('graph_learn_num_pers', 1),
                                            metric_type=config.get('graph_metric_type', 'weight_cosine'),
                                            device=self.device)


            self.graph_learner2 = graph_learn_fun(input_hidden_size,
                                            config.get('graph_learn_hidden_size2', config.get('graph_learn_hidden_size', 16)),
                                            topk=config.get('graph_learn_topk2', None),
